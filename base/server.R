...
if(nrow(all1)>0){
  ...
  all2<-spread(all1,key="nombre", value="nombre_escala")
  all3<-spread(all2, key="nombre_factor",value = "nombre_nivel")
  all<-spread(all3, key="nombre_dato",value = "valor")
  ...
  # rows per study 
  dxe<-aggregate(all[,1], list(exp=all$nombre_experimento), length)
  ...
  dxv<-aggregate(all[,1], list(exp=all$nombre_tipo_valor), length)
}
