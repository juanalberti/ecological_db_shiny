...
menu<-dashboardSidebar(
  ...
  menuItem("New variable & data type", icon = icon("ruler"),href = "../alta_tvalor/", newtab = F),
...
inserta_inserc <- function (id_usuario, id_experimento,db) 
{
  query <- sprintf ("INSERT INTO inserciones (fk_id_usuario, fk_id_experimento) 
                    VALUES (%s, %s)", id_usuario, id_experimento)
  (data <- dbExecute(db, query))
...
inserta_registro_valor <- function (id_dato, valor, tipo_valor, id_registro,db)
{
  query <- sprintf ("INSERT INTO registro_valor (fk_id_dato, valor, fk_id_tipo_valor, fk_id_registro) VALUES (%s, %s, %s, %s)", id_dato, valor, tipo_valor, id_registro)
  (data<-dbExecute(db, query))
  return(data)
}
...
alta_tipo_dato <- function (nombre_tipo_dato) 
{
  db <- conecta()
  on.exit(dbDisconnect(db))
  query <- sprintf ("INSERT INTO tipo_dato (nombre_tipo_dato) 
                    VALUES ('%s')", nombre_tipo_dato)
  (data <- dbExecute(db, query))
...
alta_niv <- function (fk_id_factor, nombre_nivel, db) 
{
  query <- sprintf ("INSERT INTO nivel (fk_id_factor, nombre_nivel) 
                    VALUES (%s, '%s')", fk_id_factor,nombre_nivel)
  (data <- dbExecute(db, query))
  return(data)
}

alta_data <- function (nombre_dato,fk_id_tipo_dato, json_mas_data) 
{
  db <- conecta()
  on.exit(dbDisconnect(db))
  query <- sprintf ("INSERT INTO dato (nombre_dato, fk_id_tipo_dato,json_mas_data) 
                    VALUES ('%s', %s, '%s')", nombre_dato,fk_id_tipo_dato, json_mas_data)
  (data <- dbExecute(db, query))
  return(data)
}

alta_tvalor <- function (nombre_tipo_valor, unidad_medida,decimales,mostrar) 
{
  db <- conecta()
  on.exit(dbDisconnect(db))
  query <- sprintf ("INSERT INTO tipo_valor (nombre_tipo_valor, unidad_medida,decimales,mostrar) 
                    VALUES ('%s', '%s', %s, %s)", nombre_tipo_valor, unidad_medida,decimales,mostrar)
  (data <- dbExecute(db, query))
  query <- sprintf("SELECT LAST_INSERT_ID()");
  data <- dbGetQuery(db, query)
  return(data)
...
update_ins<-function(valor, fk_id_insercion, fk_id_registro,fk_id_dato,fk_id_tipo_valor,db){
  query<-sprintf("update registro_valor rv set rv.valor_previo=rv.valor,
                 ...
                 and rv.fk_id_dato=%s and rv.fk_id_tipo_valor=%s",valor, fk_id_insercion, fk_id_registro,fk_id_dato,fk_id_tipo_valor)
  (data <- dbExecute(db, query))
...
trae_insertion<-function(id_ex){
  mydb <- conecta()
  on.exit(dbDisconnect(mydb))
  ins<-suppressWarnings(dbGetQuery(mydb, sprintf("SELECT fecha_ins FROM inserciones WHERE fk_id_experimento=%s",id_ex)))
  return(ins)
}
...
trae_id_tipo_valor<-function(){
  mydb <- conecta()
  on.exit(dbDisconnect(mydb))
  TDato<-dbGetQuery(mydb, "select id_tipo_valor, nombre_tipo_valor from tipo_valor")
  return(TDato)
}
...
trae_tv<-function(columnas){
  mydb <- conecta()
  on.exit(dbDisconnect(mydb))
  query <- sprintf("select distinct(vpd.fk_id_tipo_valor), tv.nombre_tipo_valor
                   from 
                   valor_por_dato vpd
...
trae_ex_x_tv<-function(tipo_valor){
  mydb<-conecta()
  on.exit(dbDisconnect(mydb))
  query<-sprintf("select distinct(ex.nombre_experimento), ex.id_experimento from registro r
                 ...
                 where tv.id_tipo_valor IN (%s)",tipo_valor)
  data<-dbGetQuery(mydb,query)
  return(data)
}
...
trae_todo<-function(id_ex, id_tv, yrs){
  ...
  query<-sprintf("select r.id_registro,r.anio, r.id_registro_padre,e.obs_escala, e.nombre_escala,
                 ...
                 rv.valor, tv.nombre_tipo_valor, tv.mostrar
                 from registro r
                 left join registro_escala re on re.fk_id_registro = r.id_registro
                 left join escala e on e.id_escala = re.fk_id_escala
                 left join tipo_escala te on te.id_tipo_escala = e.fk_tipo_escala
                 ...
                 left join dato d on d.id_dato = rv.fk_id_dato
                 left join tipo_valor tv on tv.id_tipo_valor = rv.fk_id_tipo_valor
                 where r.fk_id_experimento in (%s) and rv.fk_id_tipo_valor in (%s) and r.anio in (%s)",
...
trae_tipo_variable<-function(){
  mydb <- conecta()
  on.exit(dbDisconnect(mydb))
  TV<-dbGetQuery(mydb, "select nombre_tipo_valor, unidad_medida, decimales from tipo_valor")
  return(TV)
}
